...
export function LinkCard({ link, setLinks, isMobile }: LinkCardProps) {
  ...
  return (
    <Card className={`overflow-hidden ${isMobile ? 'flex' : ''}`}>
      <LinkCardImage link={link} isMobile={isMobile} />
...
function LinkCardImage({ link, isMobile }: { link: Link; isMobile: boolean }) {
  return (
    <div
      className={`${isMobile ? 'w-1/3 flex-shrink-0' : 'w-full aspect-video'}`}
    >
